...
public class CineStarController extends WebMvcAutoConfiguration {
	...
	public Pelicula getPelicula(@PathVariable("id") int id){
		Pelicula pelicula=(Pelicula) iPelicula.getPelicula(id);
		String generoActual = pelicula.getGeneros();
	    String generoNuevo = jdbcTemplate.queryForObject("SELECT getGenerosDetalle(?)", String.class, generoActual);
	    pelicula.setGeneros(generoNuevo);
	    return pelicula;
	}
